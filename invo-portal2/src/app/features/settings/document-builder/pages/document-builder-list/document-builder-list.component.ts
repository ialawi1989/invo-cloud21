...
export class DocumentBuilderListComponent implements OnInit {
   ...
   *  namespace finishes loading. */
  private i18nTick = signal(0);

  constructor() {
    withTranslations('settings/document-builder');
  ...
  /** Settings → Document type breadcrumbs. The document type comes
   *  from the URL `?type=` param, so the second crumb localises off
   *  the type key — switching between entities (Invoice / Bill /…)
   *  changes the label without a route reload. */
  breadcrumbs = computed<BreadcrumbItem[]>(() => {
    this.i18nTick();
    return [
      { label: this.translate.instant('SETTINGS.TITLE'), routerLink: '/settings' },
      { label: this.translate.instant('DOCUMENT_BUILDER.TYPE.' + this.documentType()) },
    ];
  });
