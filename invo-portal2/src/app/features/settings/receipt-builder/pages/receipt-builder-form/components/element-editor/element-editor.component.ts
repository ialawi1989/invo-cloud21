...
export class ElementEditorComponent {
   ...
   * the "Use Feedback link" preset. Delegates the host resolution to
   * `StorefrontUrlService` (custom domain → slug-on-invopos-shop
   * scoped by tier) so this and every other "open the storefront"
   * action share one source of truth. The POS swaps `!invoice.id`
   * for the real id at print time.
   */
  feedbackUrl(): string {
    return this.storefront.pageUrl('feedback?invoice=!invoice.id');
  }
