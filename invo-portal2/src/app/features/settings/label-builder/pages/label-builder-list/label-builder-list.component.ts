...
export class LabelBuilderListComponent implements OnInit {
  ...
  /** Re-runs translation-dependent `computed`s once the feature
   *  namespace finishes loading. Without it, `translate.instant()`
   *  is called once before the bundle lands and returns the raw key
   *  (e.g. "LABEL_BUILDER.LIST_TITLE") with no later recomputation. */
  private i18nTick = signal(0);

  constructor() {
    withTranslations('settings/label-builder');
  ...
  /** Settings → Label builder breadcrumbs. Rendered by `<app-list-shell>`
   *  when passed via `[breadcrumbs]`. */
  breadcrumbs = computed<BreadcrumbItem[]>(() => {
    this.i18nTick();
    return [
      { label: this.translate.instant('SETTINGS.TITLE'),         routerLink: '/settings' },
      { label: this.translate.instant('LABEL_BUILDER.LIST_TITLE') },
    ];
  });
  ...
  /** Options for the `<app-segmented-toggle>` that replaces the
   *  legacy pill-tab filter strip. Labels are i18n keys; the toggle
   *  component runs them through the translate pipe. */
  readonly typeFilterOptions: SegmentedToggleOption<'all' | 'label' | 'kitchen'>[] = [
    { value: 'all',     label: 'COMMON.ALL' },
    { value: 'label',   label: 'LABEL_BUILDER.TYPE.LABEL' },
    { value: 'kitchen', label: 'LABEL_BUILDER.TYPE.KITCHEN' },
  ];
