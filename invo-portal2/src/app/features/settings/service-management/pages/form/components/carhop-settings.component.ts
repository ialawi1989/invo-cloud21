...
/** Per-branch settings for `type === 'CarHop'`. Same Charge / Price
 *  layout as Pickup plus an `enableDriverOptions` toggle. */
@Component({
  selector: 'app-carhop-settings',
  standalone: true,
  ...
  template: `
    ...
    <div class="bs-toggles">
      ...
      <label class="bs-toggle">
        <input type="checkbox"
          [checked]="!!branch.setting.enableDriverOptions"
          (change)="patchSetting('enableDriverOptions', $any($event.target).checked)"/>
        <span class="bs-toggle__text">
          <strong>{{ 'SERVICE_MANAGEMENT.FIELDS.ENABLE_DRIVER_OPTIONS' | translate }}</strong>
        </span>
      </label>
    </div>
  `,
...
export class CarhopSettingsComponent {
