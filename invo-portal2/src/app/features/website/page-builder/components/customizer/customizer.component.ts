    ...
    <div class="customizer-layout">
      ...
      <header class="customizer-header">
        <div class="header-left">
          <button class="btn-icon" title="Back" (click)="back.emit()">
...
export class CustomizerComponent {
  ...
  /**
   * Saving belongs to the host: the editor state is one field on a page row,
   * and the host owns that row. It flips `saving` and reports the outcome.
   */
  handleSave(): void {
    this.save.emit();
  }

  /** Called by the host once the row is persisted. */
  savedOk(message = 'Changes saved!'): void {
    this.showNotification(message, 'success');
  }

  saveFailed(message = 'Could not save the page'): void {
