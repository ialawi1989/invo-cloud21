...
export class PreviewFrameComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  /** Slug of the page being edited, so the iframe opens THAT page rather than
   *  the storefront home. */
  @Input() pageSlug = '';
  ...
  constructor(
    ...
    private storefront: StorefrontUrlService,
  ) {}

  ngOnInit(): void {
    // Through StorefrontUrlService so local / LAN / dev / prod and custom
    // domains all resolve — and so the local preview carries ?tenant=<slug>,
    // without which the storefront can't tell which company to load.
    const base = this.storefront.pageUrl(this.pageSlug ? `/${this.pageSlug}` : '');
    this.previewUrl = base;
    const url = base + (base.includes('?') ? '&' : '?') + 'customize=true';
