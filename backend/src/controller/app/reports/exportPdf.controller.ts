...
export class pdfExportController {
  ...
  public static async export(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (reportData) {
        let resData: any;
        // convert camel case to space case with uppercase for file name with timestamp
        reportData.data.fileName = req.url.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/\//g, '') + "_" + new Date().getTime();
        if (agingReports.includes(report.toLowerCase())) {
          ...
          reportData.data.options.fileName = req.url.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/\//g, '') + "_" + new Date().getTime();
        ...
        if (resData) {
          ...
          if (resData.fileName && fs.existsSync(resData.fileName)) {
            fs.unlinkSync(resData.fileName);
          }

          res.setHeader(
            'Content-Disposition',
            `attachment; filename="${resData.fileName}"`
          );

          res.setHeader('Content-Type', 'application/pdf');

          const fileBuffer = Buffer.from(resData.buffer, 'base64');
        ...
        res.send(new ResponseData(false, "", []))
      }
  ...
  public static async export2(req: Request, res: Response, next: NextFunction) {
    try {
      const company = res.locals.company;
      const data = req.body;
      const branches = res.locals.branches
      ...
      switch (report.toLowerCase()) {
        case "/salessummary":
          ...
          let reportRecords = {
            ...
            " ": {
              records: [{ "key": "Refund", "": "", "value": refund }, { "key": "Payout", "": "", "value": payout }, { "key": "Net Income", "": "", "value": totalActualAmountPaid - refund - payout }],
        ...
        case "/vatreport":
          vatReport = true
          reportData = (await vatReportRepo.exportTaxReport(data, company, branches)).data;
          break;
      ...
      if (reportData) {
        const resData = await ReportsPDFGenerator.exportPdf2(reportData, company, vatReport ? { vatReport: vatReport } : undefined);
        // Send the file as a response
  ...
  public static async exportBalanceSheet(req: Request, res: Response, next: NextFunction) {
    try {
      const company = res.locals.company;
      const data = req.body;
      const branches = res.locals.branches
      ...
      reportData = (await ComparisonReportsRepo.balanceSheetReport(data, company, branches)).data

      if (reportData) {
        const resData = await ReportsPDFGenerator.exportBalanceSheetPdf(reportData, company);
        // Send the file as a response
  ...
  public static async exportProfitAndLoss(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      reportData = (await ComparisonReportsRepo.getProfitAndLossReport(data, company, branches)).data

      if (reportData) {
        const resData = await ReportsPDFGenerator.exportProfitAndLoss(reportData, company);
        // Send the file as a response
