...
export class PackageProductRepo {

  public static async addPackage(client: PoolClient, data: any, company: Company, employeeId: string) {
    try {
      const companyId = company.id;
      const validate = await ProductValidation.packageValidation(data);
      if (!validate.valid) {
      ...
      product.package.forEach((element: any) => {
      ...
      const query: { text: string, values: any } = {
        text: `INSERT INTO "Products"
                   ...
                   translation, "categoryId", tags, type, warning,
                   "package", "companyId","productMedia","priceModel","commissionPercentage","commissionAmount",color,"taxId","isDiscountable",  "mediaId","updatedDate","sku","alternativeProducts","maxItemPerTicket","kitchenName","reorderPoint","reorderLevel","productDeduction","logs","brandid","comparePriceAt","customFields","isSaleItem","isPurchaseItem","threeDModelId","categoryIndex","tabBuilder")
        ...
        values: [
          ...
          product.type,
          product.warning,
          JSON.stringify(product.package),
          product.companyId,
  ...
  public static async editPackage(client: PoolClient, data: any, company: Company, employeeId: string) {
    ...
    try {
      ...
      for (let index = 0; index < product.branchProduct.length; index++) {
        ...
        if (branchProduct.id == null || branchProduct.id == "") {
          ...
          const updateToBranch = await BranchProductsRepo.editBranchProduct(client, branchProduct, companyId, afterDecimal, tempPrice, employeeId, currencySymbol)
        }
