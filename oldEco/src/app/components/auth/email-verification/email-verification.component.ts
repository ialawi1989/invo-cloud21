...
export class EmailVerificationComponent implements OnInit, OnDestroy {
  ...
  private readonly OTP_TIMER_KEY = 'email_otp_timer_expiry';
  ...
  loadData(data: any) {
    if (data.email) {
      this.verificationData.email = data.email || '';
    }
  }
  ...
  verifyStep1() {
    ...
    if (!this.verificationData.email) {
      this.alertService.showAlert({ title: "Please enter valid email" });
      return;
    }
  ...
  verifyStep2() {
    ...
    this.authService.validateOtp({
      ...
      error: (err: any) => {
        this.loadingService.hideLoadingSpinner();
        this.logger.error(err?.message, { stack: err?.stack, context: 'EmailVerificationComponent' });
      },
    });
  }
