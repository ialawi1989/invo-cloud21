...
export class LoggerHelper {
    ...
    private async sendEvent(payload: unknown): Promise<void> {
        // Skip outbound shipping when the logger isn't configured (empty project
        // key or placeholder endpoint). Otherwise every error event would spam
        // EPROTO/ECONNREFUSED failures against the placeholder URL.
    ...
    /**
     * Forward a pre-formed payload from another source (e.g. the browser-side
     * LoggerService relayed via POST /api/log) to the upstream ingest endpoint.
     * Caller is responsible for the payload shape; we only attach server-side
     * context that the browser cannot see (e.g. forwardedFor).
     */
