...
@Component({
  ...
  selector: 'top-item-by-sales',
  templateUrl: './top-item-by-sales.component.html',
  styleUrls: ['./top-item-by-sales.component.scss']
})
export class TopItemBySalesComponent implements OnInit, OnDestroy, OnChanges {
  @ViewChild('topItemBySalesChart', { static: true }) topItemBySalesChart!: ChartComponent;
  ...
  async loadData() {
    ...
    let response = await this.dashboardService.topItemBySales(data);

    this.topItemBySales.series[0].data = [];
    this.topItemBySales.xaxis.categories = [];

    response.forEach((item: any) => {
      let sales = parseFloat(item.sales);
      let productName = item.productName;
      this.topItemBySales.series[0].data.push(sales);
      this.topItemBySales.xaxis.categories.push(productName);
    });

    if (this.topItemBySales.series.length) {
      // Update the chart by calling the updateSeries and updateOptions methods
      this.topItemBySalesChart.updateSeries(this.topItemBySales.series);
      this.topItemBySalesChart.updateOptions(this.topItemBySales);
    } else {
      this.topItemBySalesChart.updateSeries([]);
      this.topItemBySalesChart.updateOptions({});
    }
